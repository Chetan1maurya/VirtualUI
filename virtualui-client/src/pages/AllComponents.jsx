  ...
  return (
    <div className="flex flex-col items-center justify-center h-full px-6 sm:px-8 text-center py-10 sm:py-16">
      <motion.div
        ...
        className="w-full"
      >
        <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-2xl bg-[#3be8ff]/7 border border-[#3be8ff]/15 flex items-center justify-center mx-auto mb-5 sm:mb-6">
        ...
        <div className="w-full max-w-md mx-auto text-left space-y-4 mb-8">
          ...
          <div>
            ...
            <CodeBlock code={`npm install "virtual-library-ui"`} />
          </div>
          ...
          <div>
            ...
            <CodeBlock
              code={`import {componentName} from virtual-library-ui`}
              lang="jsx"
            />
          </div>
          ...
          <div>
            ...
            <CodeBlock
              code={`import { UserAvatar, PricingCard } from "virtual-library-ui";\n\nexport default function App() {\n return (\n   <div>\n     <UserAvatar src="/User.png" />\n     <PricingCard title="Pro" price={99} />\n   </div>\n  );\n}`}
              lang="jsx"
            />
          </div>
        </div>
...
function DetailPanel({ component, onBack }) {
  ...
  const importCode = `import { ${component.name} } from "virtual-library-ui";`;

  const usageCode = `import { ${component.name} } from "virtual-library-ui";\n\nexport default function App() {\n  return (\n   <div>\n     <${component.name}${component.props?.length ? `\n     ${component.props.map((p) => `${p}={/* value */}`).join("\n       ")}` : ""} />\n    </div>\n   );\n}`;

  return <motion.div key={component._id} 
    ...
    <div className="flex items-start sm:items-center justify-between px-4 sm:px-6 py-4 sm:py-5 border-b border-white/6 gap-3 flex-wrap">
      <div className="flex items-center gap-3 min-w-0">
        {onBack && (
          <button 
          onClick={onBack}
          className="sm:hidden flex items-center justify-center w-8 h-8 rounded-xl bg-white/5 border border-white/8 text-white/50 hover:text-white/50 transition-colors cursor-pointer shrink-0">
            <TbX size={13} />
          </button>
        )}
        <div className="min-w-0">
          ...
          <p className="text-white/35 text-[11px] sm:text-xs mt-0.5 truncate">
            {
              component.props?.length > 0 ? `Props: ${component.props.join(", ")}` : "No props"
            }
          </p>
        </div>
      </div>

      <div className="flex gap-1 rounded-xl p-1 overflow-x-auto shrink-0" style={{ background: "rgba(0,0,0,0.3)" }}>
            {
              [
                {
        ...
        {activeTab === "guide" && (
          <motion.div key="guide" initial={{opacity: 0}} animate={{opacity: 1}} exit={{opacity: 0}} className="space-y-5">
              ...
              <div>
                ...
                <CodeBlock code={`npm install virtual-library-ui`} lang="bash"/>
              </div>
...
const AllComponents = () => {
  ...
  return (
    ...
    >
      {/* Navbar */}
      <nav className="sticky top-0 z-40 flex items-center justify-between px-4 sm:px-8 py-3.5 sm:py-4 border-b border-white/5 bg-[#030b0d]/90 backdrop-blur-md shrink-0">
        <button
          onClick={() => navigate("/")}
          ...
          {sidebarOpen && (
            <>
              ...
              <motion.div
                ...
                className="sm:hidden fixed top-0 left-0 z-50 h-full w-72 flex flex-col bg-[#040e11] border-r border-white/8"
              >
                <div className="flex items-center justify-between px-4 py-4 border-b border-white/6">
                  <span className="text-xs font-bold uppercase text-white/40 tracking-widest">
                    Components
                  </span>
                  <button
                    onClick={() => setSidebarOpen(false)}
