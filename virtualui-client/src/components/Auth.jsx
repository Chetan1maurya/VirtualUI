...
const steps = [
    ...
    {icon: TbCopy, title: "Generate Componenets", desc: "AI builds production-ready JSX components."},
  ...
  return (
    ...
    className="fixed inset-0 flex items-center justify-center bg-black/70 backdrop-blur-sm z-50 p-4">
        ...
        className="flex flex-col sm:flex-row w-full max-w-220 max-h-[90vh] overflow-y-auto rounded-2xl border border-[#3be8ff]/10 shadow-[0_40px_80px_rgba(0,0,0,0.8)] relative">
            <button onClick={onClose} className="absolute top-3 right-3 z-20 w-8 h-8 rounded-lg bg-white/5 hover:bg-white/10 flex items-center justify-center text-white/50 hover:text-white transition-all cursor-pointer"><TbX size={15}/></button>
            {/* left-box */}
            <div className="sm:w-[52%] bg-linear-to-br from-[#03181c] to-[#041e24] p-6 sm:p-10 relative overflow-hidden">
                <div className="absolute -top-10 -right-16 w-64 h-64 rounded-full bg-[radial-gradient(circle,rgba(59,232,255,0.08)_0%,transparent_70%)] pointer-events-none"/>
                ...
                className="flex items-center gap-3 mb-7 sm:mb-9">
                    ...
                    <span className="text-xl font-bold text-[#e8f8fa] tracking-tight" style={{ fontFamily:"'Syne',sans-serif" }}>VirtualUI</span>
                </motion.div>
                ...
                <div className="flex sm:flex-col gap-2 sm:gap-1 overflow-x-auto sm:overflow-x-visible pb-2 sm:pb-0 -mx-1 px-1">
                    {
                    steps.map((step,i)=>(
                        <motion.div key={i} className={`shrink-0 sm:shrink flex items-start gap-3 px-3 py-2.5 rounded-xl border transition-all duration-300 min-w-50 sm:min-w-0 ${active === i ? "bg-[#3be8ff]/[0.07] border-[#3be8ff]/20": "bg-transparent border-transparent"}`}>

                            <div className={`min-w-7 h-7 rounded-lg flex items-center justify-center border transition-all duration-300 ${active === i ? "bg-linear-to-br from-[#3be8ff] to-[#0ab8d6] border-transparent" : "bg-[#3be8ff]/8 border-[#3be8ff]/20"}`}>
                                <step.icon size={17} color={active === i ? "#051c20" : "#3be8ff"} />
                            </div>

                            <div>
                                <p className={`text-[12.5px] font-semibold transition-colors duration-300 whitespace-nowrap sm:whitespace-normal ${active === i ? "text-[#d4f5fa]" : "text-white/55"}`}>{step.title}</p>
                                <div className={`overflow-hidden transition-all duration-500 ${active === i ? "max-h-8 opacity-100 mt-0.5" : "max-h-0 opacity-0"}`}><p className="text-[11px] text-[#3be8ff]/40 leading-relaxed">{step.desc}</p>
                                </div>
            ...
            className="sm:w-[48%] bg-[#040f12] px-6 sm:px-10 py-8 sm:py-12 flex flex-col justify-center items-center relative overflow-hidden">
                ...
                <div className="relative z-10 w-full max-w-70 sm:max-w-65 text-center mx-auto">
                    <motion.div 
                    animate={{y:[0,-6,0]}}
                    transition={{duration:3, repeat: Infinity, ease: "easeInOut"}}
                    className="w-12 h-12 sm:w-14 sm:h-12 rounded-2xl mx-auto mb-5 sm:mb-6 bg-linear-to-br from-[#3be8ff]/15 to-[#040f12] border border-[#3be8ff]/20 flex items-center justify-center">
                    ...
                    <p className="text-[11px] text-[#64919b]/45 mt-4 sm:mt-5">
                        No account needed for npm.{" "} <span onClick={onClose} className="text-[#3be8ff]/50 border-b border-[#3be8ff]/20 cursor-pointer hover:text-[#3be8ff]/8 transition-colors">View docs</span>
                    </p>
                </div>
            </motion.div>
        </motion.div>
